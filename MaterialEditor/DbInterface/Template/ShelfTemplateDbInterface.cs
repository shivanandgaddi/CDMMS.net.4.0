...
{
    public class ShelfTemplateDbInterface : TemplateDbInterfaceImpl
    {
        private static Logger logger = LogManager.GetCurrentClassLogger();

        public ShelfTemplateDbInterface() : base()
        {

        }

        public ShelfTemplateDbInterface(string dbConnectionString) : base(dbConnectionString)
        {
        ...
        public override ITemplate GetTemplate(long templateId, bool isBaseTemplate)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (template != null)
                        ((OverAllShelfTemplate)template).TemplateBase = (BaseTemplate)GetBaseTemplate(((OverAllShelfTemplate)template).BaseTemplateId);
                }
            }
            catch (Exception ex)
            {
        ...
        private ITemplate GetBaseTemplate(long templateId)
        {
            ...
            IDataReader reader = null;
            ITemplate template = null;
            ShelfSpecificationDbInterface dbInterface = null;
            ...
            try
            {
                ...
                reader = dbManager.ExecuteDataReaderSP("base_tmplt_pkg.get_base_shelf_tmplt", parameters);
                ...
                if (reader.Read())
                {
                    template = new BaseShelfTemplate(templateId);
                    ...
                    template.SpecificationRevisionId = long.Parse(DataReaderHelper.GetNonNullValue(reader, "shelf_specn_revsn_alt_id", true));
        ...
        private ITemplate GetOverAllTemplate(long templateId)
        {
            ...
            try
            {
                ...
                reader = dbManager.ExecuteDataReaderSP("OVERL_SHELF_PKG.get_overl_shelf_tmplt_1", parameters);
                ...
                if (reader.Read())
                {
                    template = new OverAllShelfTemplate(templateId);
                    dbInterface = new ShelfSpecificationDbInterface();
                    ...
                    ((OverAllShelfTemplate)template).BaseTemplateId = long.Parse(DataReaderHelper.GetNonNullValue(reader, "base_tmplt_id", true));
                    //((OverAllShelfTemplate)template).BayExtndrSpecnRevsnAltId = long.Parse(DataReaderHelper.GetNonNullValue(reader, "bay_extndr_specn_revsn_alt_id", true));
        ...
        public long CreateBaseTemplate(long specRvsnId, string name, string description)
        {
            ...
            try
            {
                ...
                id = CreateTemplateInTmpltTbl(name, description, "Shelf", true);

                shelfParameters = dbAccessor.GetParameterArray(2);

                shelfParameters[0] = dbAccessor.GetParameter("pTmpltId", DbType.Int64, id, ParameterDirection.Input);
                shelfParameters[1] = dbAccessor.GetParameter("pBaySpecnRevsnAltId", DbType.Int64, specRvsnId, ParameterDirection.Input);
                ...
                dbAccessor.ExecuteNonQuery(CommandType.StoredProcedure, "base_tmplt_pkg.insert_base_shelf_tmplt", shelfParameters);
        ...
        public long CreateOverAllTemplate(long baseTmpltId, string name, string description, string cuid)
        {
            ...
            try
            {
                ...
                id = CreateTemplateInTmpltTbl(name, description, "Shelf", false);
